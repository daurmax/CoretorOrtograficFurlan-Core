...
using CoretorOrtografic.Core.Constants;
using CoretorOrtografic.Core.Enums;
using CoretorOrtografic.Core.Extensions;
using CoretorOrtografic.Core.FurlanPhoneticAlgorithm;
using CoretorOrtografic.Core.KeyValueDatabase;
using CoretorOrtografic.Core.SpellChecker;
using CoretorOrtografic.Dictionaries.Constants;
using System;
...
namespace CoretorOrtografic.Infrastructure.SpellChecker
{
    public class FurlanSpellChecker : ISpellChecker
    {
        private readonly ILogger<FurlanSpellChecker> _logger;
        ...
        public FurlanSpellChecker(IKeyValueDatabase keyValueDatabase, ILogger<FurlanSpellChecker> logger)
        {
        ...
        {
            _logger.LogInformation("Starting spell check for text: {Text}", text);
            ...
            foreach (ProcessedWord word in ProcessedWords)
            {
                try
                {
                    ...
                    {
                        _logger.LogInformation("Checking word: {Word}", word.Original);
                        word.Correct = CheckWord(word).Result;
                        if (!word.Correct)
                        {
                            _logger.LogInformation("Word is incorrect: {Word}", word.Original);
                            word.Suggestions = GetWordSuggestions(word).Result.ToList();
                            _logger.LogInformation("Suggestions for {Word}: {Suggestions}", word.Original, string.Join(", ", word.Suggestions));
                        }
                    }
                    else
                    {
                ...
                {
                    _logger.LogError(ex, "Error checking word: {Word}", word.Original);
                    word.Correct = false;
        ...
        public async Task<bool> CheckWord(ProcessedWord word)
        {
            ...
            return await Task.Run(async () =>
            {
                ...
                {
                    _logger.LogInformation("Word contains digits or invalid characters: {Word}", wordAsString);
                    return true;
                }
                ...
                {
                    _logger.LogInformation("Word found in suggestions: {Word}", wordAsString);
                    return true;
                }
        ...
        {
            _logger.LogInformation("Getting suggestions for word: {Word}", word.Original);
            ...
            _logger.LogInformation("Suggestions for {Word}: {Suggestions}", word.Original, string.Join(", ", sortedSuggestions.Take(MAX_SUGGESTIONS)));
        ...
        {
            _logger.LogInformation("Suggesting raw for word: {Word}", word.Original);
            if (word == null) throw new ArgumentNullException(nameof(word));
        ...
        {
            _logger.LogInformation("Building suggestions for word: {Word}", word.Original);
            if (word == null) throw new ArgumentNullException(nameof(word));
        ...
        {
            _logger.LogInformation("Getting basic suggestions for word: {Word}", word.Original);
            if (word == null) throw new ArgumentNullException(nameof(word));
        ...
        {
            _logger.LogInformation("Getting system dictionary phonetic suggestions for word: {Word}", word.Original);
        ...
        {
            _logger.LogInformation("Getting user dictionary phonetic suggestions for word: {Word}", word.Original);
            return await Task<ICollection<string>>.Factory.StartNew(() =>
            {
                (string, string) wordHashes = FurlanPhoneticAlgorithm.GetPhoneticHashesByWord(word.ToString());
        ...
        {
            _logger.LogInformation("Finding in exceptions for word: {Word}", word.Original);
        ...
        {
            _logger.LogInformation("Getting radix tree suggestions for word: {Word}", word.Original);
            return await Task<ICollection<string>>.Factory.StartNew(() =>
            {
                var _rt = new CoretorOrtografic.Core.RadixTree.RadixTree(DictionaryFilePaths.WORDS_RADIX_TREE_FILE_PATH);
